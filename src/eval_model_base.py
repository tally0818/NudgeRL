...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=f"Evaluate a base Hugging Face model on selected datasets ({DEFAULT_DATASETS_ARG})."
    )
    parser.add_argument("--model-path", type=str, required=True, help="Path or HuggingFace ID of the base model (e.g., Qwen/Qwen3-4B)")
    parser.add_argument(
        "--datasets",
        type=str,
...
def print_summary(
    *,
    ...
    print(f"\n=== {dataset_name} Evaluation (Base Model) ===")
    print(
...
def main() -> None:
    ...
    print(f"Launching vLLM for base model: {model_path}")
    ...
    payload: dict[str, Any] = {
        ...
        "results_by_dataset": {
            dataset_name: {"model_ref": model_path, **result}
